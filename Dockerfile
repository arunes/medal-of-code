...
RUN apt-get update && apt-get install curl -y && curl -sL https://deb.nodesource.com/setup_22.x | bash -
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/moc ./
...
CMD ["sh", "-c", "/app/bin/moc eval Moc.Release.migrate && /app/bin/server"]
