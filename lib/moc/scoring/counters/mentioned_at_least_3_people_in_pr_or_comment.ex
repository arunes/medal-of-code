defmodule Moc.Scoring.Counters.MentionedAtLeast3PeopleInPROrComment do
  ...
  defp get_result_for_pr(created_by_id, description) do
    ...
    cond do
      length(mentions) >= 3 -> [%{contributor_id: created_by_id, count: 1}]
      true -> []
    end
  end
  ...
  defp get_result_for_comments(comments) do
    ...
    |> Enum.filter(fn cmt -> length(cmt.mentions) >= 3 end)
