defmodule Moc.Tests.Counters.PrsApprovedWithSuggestionsTest do
  use ExUnit.Case
  alias Moc.Scoring.Counters.PrsApprovedWithSuggestions
  ...
  test "returns an empty list when there are no reviews" do
    ...
    assert PrsApprovedWithSuggestions.count(input, nil) == []
  end
  ...
  test "returns a contributor with a count of 1 when the PR is approved with suggestions" do
    input = %Type.Input{
      created_by_id: 1,
      reviews: [
  ...
  test "returns multiple contributors with a count of 1 when the PR is approved with suggestions by multiple reviewers" do
    input = %Type.Input{
      created_by_id: 1,
      reviews: [
        %{vote: 5, reviewer_id: 2},
